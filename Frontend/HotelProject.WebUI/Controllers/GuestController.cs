...
namespace HotelProject.WebUI.Controllers
{
    public class GuestController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:5195/api/Guest");
        ...
        public async Task<IActionResult> AddGuest(CreateGuestDto createGuestDto) 
        {
			...
			if (result.IsValid)
			{
                ...
                var responseMessage = await client.PostAsync("http://localhost:5195/api/Guest", stringContent);
                if (responseMessage.IsSuccessStatusCode)
                {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5195/api/Guest/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateGuest(UpdateGuestDto updateGuestDto)
        {
			...
			if (result.IsValid)
            {
                ...
                StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                var responseMessage = await client.PutAsync("http://localhost:5195/api/Guest/", stringContent);
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("Index");
                }
